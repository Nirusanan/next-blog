        ...
        <main className="bg-cover bg-center min-h-screen" style={{ backgroundImage: "url('/img/background.jpg')" }}>
            <div className="container mx-auto px-16 py-16 w-3/4">
                <div className="bg-white shadow-lg rounded-lg p-8 bg-opacity-90 backdrop-blur-sm">
                    <h2 className="text-center text-3xl sm:text-4xl font-bold mb-4 text-gray-800">{postData.post.title}</h2>
                    ...
                    <div className="my-6 text-lg text-gray-700 leading-relaxed">
                        {postData.post.description.split('\n').map((text, index) => (
                            <p key={index} className={`mx-4 text-justify whitespace-pre-line ${index === 0 ? 'mt-0' : 'mt-4'}`}>{text}</p>
                        ))}
                    </div>
                    <p className="text-right text-gray-500">Author: {postData.post.userId.name}</p>
                    <p className="text-right text-gray-500">{postData.post.created_at_formatted}</p>
                    <div className="mt-8 flex items-center">
                        ...
                        <span className="ml-2 text-lg">{postData.likes}</span>
                    </div>
